...
export function ContributorSignupForm({ override }: { override?: RedactoresContent }) {
  ...
  if (state === "ok") {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-gold/40 bg-surface/80 p-8 text-center backdrop-blur">
        ...
        <p className="mt-2 max-w-sm text-sm text-muted-foreground">{c.success_text}</p>
        <button
          type="button"
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <input name="full_name" placeholder={c.form_placeholder_name} className={inputCls} required maxLength={120} />
        <input name="email" type="email" placeholder={c.form_placeholder_email} className={inputCls} required maxLength={255} />
        <input name="country" placeholder={c.form_placeholder_country} className={inputCls} required maxLength={80} />
        <input name="region" placeholder={c.form_placeholder_region} className={inputCls} maxLength={120} />
        <input
          name="club_or_federation"
          placeholder={c.form_placeholder_club}
        ...
        <input
          name="topics"
          placeholder={c.form_placeholder_topics}
          className={`${inputCls} md:col-span-2`}
          required
          maxLength={500}
        />
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
        <p className="text-[11px] text-muted-foreground">{c.form_consent}</p>
        <button
          type="submit"
        ...
        >
          {state === "sending" ? (
            ...
            <>
              {c.form_submit} <ArrowRight className="h-4 w-4" />
            </>
          )}
        </button>
      </div>
