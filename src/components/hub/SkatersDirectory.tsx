...
export function SkatersDirectory({ country }: { country: string }) {
  ...
  const { skaters, loading } = useSkaters(country, filters);

  const specialties = useMemo(() => {
    const set = new Set<string>();
    skaters.forEach((s) => s.specialty && set.add(s.specialty));
    ...
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-8">
      <header className="mb-6">
        ...
        <p className="mt-2 max-w-2xl text-sm text-[#B5B5B5]">
          Atletas del patinaje español. Perfiles con palmarés, marcas personales, club y todo su contenido relacionado.
        </p>
      </header>
