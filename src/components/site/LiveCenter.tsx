...
function Leaderboard({ rows, expandedId, setExpandedId, copy }: { rows: EnrichedResult[]; expandedId: string | null; setExpandedId: (id: string | null) => void; copy: LiveCenterHomeSetting }) {
  return (
    <article className="live-panel overflow-hidden rounded-lg border border-border bg-surface">
      <div className="flex items-center justify-between border-b border-border px-4 py-3"><h3 className="font-display text-xl uppercase tracking-widest">{copy.leaderboard_title}</h3><span className="font-condensed text-[10px] uppercase tracking-widest text-muted-foreground">{rows.length} {copy.riders_label}</span></div>
      {rows.length === 0 ? <div className="p-5 text-sm text-muted-foreground">{copy.empty_results_text}</div> : <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm"><thead className="font-condensed bg-background/60 text-[10px] uppercase tracking-widest text-muted-foreground"><tr><th className="px-3 py-3">Pos</th><th className="px-3 py-3">#</th><th className="px-3 py-3">País</th><th className="px-3 py-3">Patinador</th><th className="px-3 py-3">Club</th><th className="px-3 py-3">Tiempo</th><th className="px-3 py-3">Gap</th><th className="px-3 py-3">Última vuelta</th><th className="px-3 py-3">Estado</th></tr></thead><tbody className="divide-y divide-border">{rows.map((row) => <Row key={row.id} row={row} expanded={expandedId === row.id} onToggle={() => setExpandedId(expandedId === row.id ? null : row.id)} copy={copy} />)}</tbody></table></div>}
    </article>
  );
}

function Row({ row, expanded, onToggle, copy }: { row: EnrichedResult; expanded: boolean; onToggle: () => void; copy: LiveCenterHomeSetting }) {
  return <><tr onClick={onToggle} className={`cursor-pointer transition hover:bg-muted/70 ${row.position % 2 === 0 ? "bg-background/20" : ""} ${row.is_highlighted || row.position === 1 ? "live-row-pulse" : ""}`}><td className={`font-display px-3 py-3 text-xl ${row.position <= 3 ? "text-live-orange" : "text-foreground"}`}>{medal(row.position)} {row.position}</td><td className="px-3 py-3 font-mono text-xs text-muted-foreground">{row.dorsal}</td><td className="px-3 py-3 text-lg">{row.flag}</td><td className="px-3 py-3 font-semibold leading-tight">{row.athlete_name}</td><td className="font-condensed px-3 py-3 text-xs uppercase tracking-wide text-muted-foreground">{row.club ?? "—"}</td><td className="px-3 py-3 font-mono text-xs text-foreground">{row.time ?? "—"}</td><td className="px-3 py-3 font-mono text-xs text-muted-foreground">{row.gap ?? "—"}</td><td className="px-3 py-3 font-mono text-xs text-live-cyan">{row.lastLap}</td><td className="px-3 py-3"><StatusChip tone={row.statusTone}>{row.statusLabel}</StatusChip></td></tr>{expanded && <tr className="bg-background/50"><td colSpan={9} className="p-4"><ExpandedAthlete row={row} copy={copy} /></td></tr>}</>;
}

function ExpandedAthlete({ row, copy }: { row: EnrichedResult; copy: LiveCenterHomeSetting }) {
  return <div className="grid gap-4 md:grid-cols-4"><Metric label="Mejor vuelta temporada" value={row.seasonBest} /><Metric label="Promedio vueltas" value={row.avgLap} /><Metric label="Velocidad media" value={row.speed} /><button type="button" onClick={() => toast("Perfil completo próximamente")} className="live-mini-button min-h-14">{copy.profile_button_label} <ExternalLink className="h-4 w-4" /></button><div className="md:col-span-2"><p className="font-condensed mb-2 text-[10px] uppercase tracking-widest text-muted-foreground">Consistencia últimas 5 vueltas</p><div className="flex h-16 items-end gap-2">{row.consistency.map((value, index) => <span key={index} className="flex-1 rounded-t bg-live-cyan/70" style={{ height: `${value}%` }} />)}</div></div><div className="md:col-span-2"><p className="font-condensed mb-2 text-[10px] uppercase tracking-widest text-muted-foreground">Historial de posición</p><div className="flex h-16 items-end gap-1">{row.positions.map((value, index) => <span key={index} className="flex-1 rounded-t bg-live-orange/80" style={{ height: `${Math.max(18, 100 - value * 11)}%` }} />)}</div></div></div>;
}

function StatsPanel({ stats, leader, copy }: { stats: ReturnType<typeof makeStats>; leader: EnrichedResult | undefined; copy: LiveCenterHomeSetting }) {
  return <article className="live-panel rounded-lg border border-border bg-surface p-4"><h3 className="font-display mb-3 flex items-center gap-2 text-xl uppercase tracking-widest"><TrendingUp className="h-5 w-5 text-live-orange" /> {copy.stats_title}</h3><div className="grid gap-3"><Stat icon={<Zap />} label={copy.best_lap_label ?? "Mejor vuelta"} value={stats.bestLap} helper={leader?.athlete_name ?? "—"} /><Stat icon={<Gauge />} label={copy.avg_speed_label ?? "Velocidad media"} value={stats.avgSpeed} helper={`Máxima ${stats.maxSpeed}`} /><Stat icon={<Activity />} label={copy.lead_changes_label ?? "Cambios de liderato"} value={stats.leadChanges} helper={copy.current_race_helper ?? "Carrera actual"} /><Stat icon={<AlertTriangle />} label={copy.incidents_label ?? "Caídas"} value={stats.incidents} helper={copy.dq_helper ?? "Descalificaciones 0"} /></div></article>;
}

function LiveTicker({ items, copy }: { items: TickerItem[]; copy: LiveCenterHomeSetting }) {
  return <article className="live-panel rounded-lg border border-border bg-surface p-4"><h3 className="font-display mb-3 flex items-center gap-2 text-xl uppercase tracking-widest"><BarChart3 className="h-5 w-5 text-live-cyan" /> {copy.ticker_title}</h3><div className="max-h-[420px] space-y-2 overflow-y-auto pr-1">{items.slice(0, 8).map((item) => <div key={item.id} className={`ticker-card ticker-${item.tone} rounded border border-border bg-background/60 p-3`}><div className="font-condensed mb-1 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest"><span>{item.icon}</span><time className="text-muted-foreground">{item.time}</time><span>{item.type}</span></div><p className="text-sm leading-snug text-foreground/90">{item.message}</p></div>)}</div></article>;
}

function LapTimes({ rows, copy }: { rows: EnrichedResult[]; copy: LiveCenterHomeSetting }) {
  return <article className="live-panel overflow-hidden rounded-lg border border-border bg-surface"><div className="border-b border-border px-4 py-3"><h3 className="font-display text-xl uppercase tracking-widest">{copy.laps_title}</h3></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="font-condensed bg-background/60 text-[10px] uppercase tracking-widest text-muted-foreground"><tr><th className="px-3 py-3 text-left">Patinador</th><th className="px-3 py-3">V10</th><th className="px-3 py-3">V11</th><th className="px-3 py-3">V12</th><th className="px-3 py-3">Mejor</th><th className="px-3 py-3">Media</th></tr></thead><tbody className="divide-y divide-border">{rows.map((row) => <tr key={row.id} className="hover:bg-muted/70"><td className="px-3 py-3 text-left font-semibold">{row.flag} {row.athlete_name}</td>{row.consistency.slice(0, 3).map((value, index) => <td key={index} className="px-3 py-3 text-center font-mono text-xs">1:0{Math.max(7, Math.round(12 - value / 25))}.{String(200 + index * 111).slice(0, 3)}</td>)}<td className="px-3 py-3 text-center font-mono text-xs text-live-cyan">{row.seasonBest}</td><td className="px-3 py-3 text-center font-mono text-xs">{row.avgLap}</td></tr>)}</tbody></table></div></article>;
}

function HeadToHead({ rows, onOpen, copy }: { rows: EnrichedResult[]; onOpen: () => void; copy: LiveCenterHomeSetting }) {
  return <article className="live-panel rounded-lg border border-border bg-surface p-4"><div className="mb-4 flex items-center justify-between gap-3"><h3 className="font-display text-xl uppercase tracking-widest">{copy.head_title}</h3><button onClick={onOpen} className="live-mini-button">{copy.head_button_label}</button></div><div className="grid gap-3 md:grid-cols-3">{rows.map((row) => <div key={row.id} className="rounded border border-border bg-background/60 p-4"><p className="text-lg font-bold">{row.flag} {row.athlete_name}</p><p className="font-mono text-sm text-live-cyan">{row.lastLap}</p><div className="mt-3 h-2 rounded bg-muted"><div className="h-2 rounded bg-live-orange" style={{ width: `${row.consistency[0]}%` }} /></div></div>)}</div></article>;
}
...
function ComparisonModal({ rows, selected, setSelected, onClose, copy }: { rows: EnrichedResult[]; selected: string[]; setSelected: (ids: string[]) => void; onClose: () => void; copy: LiveCenterHomeSetting }) {
  ...
  return <Modal onClose={onClose} title={copy.compare_title ?? "Comparar patinadores"}><div className="grid gap-4 md:grid-cols-[240px_1fr]"><div className="max-h-80 space-y-2 overflow-y-auto">{rows.map((row) => <button key={row.id} onClick={() => toggle(row.id)} className={`w-full rounded border px-3 py-2 text-left text-sm ${selected.includes(row.id) ? "border-live-orange bg-live-orange/10" : "border-border"}`}>{row.flag} #{row.dorsal} {row.athlete_name}</button>)}</div><div className="grid gap-3">{selectedRows.length ? selectedRows.map((row) => <div key={row.id} className="rounded border border-border bg-background/60 p-3"><div className="mb-2 flex items-center justify-between gap-2"><strong>{row.athlete_name}</strong><span className="font-mono text-live-cyan">{row.lastLap}</span></div><div className="flex h-20 items-end gap-1">{row.consistency.map((value, index) => <span key={index} className="flex-1 rounded-t bg-live-orange" style={{ height: `${value}%` }} />)}</div></div>) : <p className="text-sm text-muted-foreground">{copy.compare_empty_text}</p>}</div></div></Modal>;
}
