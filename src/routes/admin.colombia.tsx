...
export const Route = createFileRoute("/admin/colombia")({
  head: () => ({
    meta: [
      { title: "Control de Colombia — RollerZone" },
  ...
  if (!hub)
    return (
      <div className="rounded-md border border-destructive/40 bg-destructive/10 p-4 text-sm">
        No se ha encontrado el hub de Colombia. Pídele al equipo que cree la fila con
        country_code = <code>co</code>.
      </div>
    );
...
function HeroEditor({ hub, onSaved }: { hub: HubRow; onSaved: () => void }) {
  ...
  return (
    <section className="rounded-xl border border-border bg-surface p-5">
      <SectionTitle title="Identidad & Hero" hint="Texto, imagen y colores del hub de Colombia." />
